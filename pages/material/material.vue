		...
		<div id="arkn-material">
			<div class="mdui-row">
				...
				<div class="mdui-col-lg-6">
					<table class="mdui-table tag-table">
						<tbody>
							<tr>
								<td v-if="!isMobile" width="1"><button class="mdui-btn mdui-btn-dense mdui-color-teal no-pe tag-btn">稀有</button></td>
								<td>
									<label v-if="isMobile" class="mdui-textfield-label">稀有度</label>
									<view :class="'mdui-btn mdui-btn-dense mdui-ripple tag-btn '+(allRare?color.selected:color.notSelected)"
									 @click="selected.rare = l.fill(Array(selected.rare.length), !allRare);" class="tag-number cu-tag bg-olive">全选</view>
									<view v-for="i in 5" :key="`rare-${rareNum+1-i}`" v-model="selected.rare[rareNum-i]" class="tag-number cu-tag bg-olive">&nbsp;{{rareNum+1-i}}&nbsp;</view>
			...
			<div class="mdui-row">
				...
				<div class="mdui-col-xs-12 mdui-m-t-4" v-if="false">
					<div class="material-group-wrap">
						...
						<div :class="isMobile?'mdui-col-xs-6 material-simple-wrap':'inline-block'" v-for="materialName in materialsOrder"
						 :key="materialName+'-simple'" v-show="showMaterialsFlatten.includes(materialName)">
							<div :class="`mdui-card ${isMobile?'mdui-center':'mdui-m-r-2'} mdui-m-b-2 material material-simple${(setting.translucentDisplay && hasInput && gaps[materialName][0]==0) ? ' opacity-5' : ''}`">
								...
								<div class="mdui-card-header" :name="materialName">
									...
									<div class="mdui-card-header-avatar mdui-valign no-sl" :t="materialsTable[materialName].rare">
										<img class="no-pe" :src="`/static/img/material/${materialsTable[materialName].img}`" />
				...
				<div class="mdui-col-xs-12" v-else v-for="i in rareNum" :key="`materials-${i}`" v-show="showMaterials[rareNum+1-i].length>0">
					...
					<div class="material-group-wrap">
						<!-- 素材卡片 -->
						<div v-for="material in materials[rareNum+1-i]" :key="material.name" v-show="showMaterials[rareNum+1-i].includes(material.name)"
						 :class="`mdui-card${isMobile?'':' mdui-m-r-2'} mdui-m-b-2 material${(setting.translucentDisplay && hasInput && gaps[material.name][0]==0) ? ' opacity-5' : ''}`">
							<div :class="`card-triangle ${color[rareNum+1-i]}`"></div>
							<div class="mdui-card-header" :name="material.name" :mdui-tooltip="isMobile?false:`{content:'合成需要：${madeofTooltips[material.name]}',position:'top'}`">
								<!-- 图片 -->
								<div class="mdui-card-header-avatar mdui-valign no-sl" :t="rareNum+1-i">
									<img class="no-pe" :src="`/static/img/material/${material.img}`" />
								</div>
								<!-- 材料名 -->
								<div :class="`mdui-card-header-title no-sl${inputs[material.name].need>0?' mdui-text-color-pink-accent':''}`">
									...
									<p v-if="isMobile" class="mdui-m-y-0 mdui-text-color-black-disabled" style="font-size:12px;font-weight:400">{{madeofTooltips[material.name]}}</p>
								</div>
								<!-- 输入面板 -->
								<div :class="isMobile?false:'mdui-m-t-1'">
				...
				<template v-if="dropDetails">
					...
					<div class="mdui-dialog-content mdui-p-b-0">
						<div class="stage" v-for="dropDetail in dropDetails" :key="`dd-${dropDetail.code}`">
							...
							<div class="num-item-list">
								<arkn-num-item v-for="drop in dropDetail.drops" :key="`detail-${dropDetail.code}-${drop[0]}`" :t="materialsTable[drop[0]].rare"
								 :img="materialsTable[drop[0]].img" :lable="drop[0]" :num="l.round(drop[1]*100,2)+'%'" :color="dropFocus==drop[0]?'mdui-text-color-black blod-text':false" />
							</div>
						</div>
					</div>
				</template>
	...
	export default {
		...
		computed: {
			...
			gaps() {
				...
				_.forIn(this.materials, (materials, i) => {
					for (const {
						...
						while (gaps[name] > 0 && _.every(madeof, (num, mName) => this.inputsInt[mName].have + made[mName] - used[mName] -
								num >= 0)) {
							gaps[name]--;
							made[name]++
			...
			plan() {
				...
				// 线性规划模型
				const useVariables = [this.setting.planIncludeEvent ? this.dropTable : _.omitBy(this.dropTable, o => o.event),
					synthesisTable.gt3
				];
				...
				// 需求狗粮
				if (this.setting.planCardExpFirst) model.variables['转换-经验值'] = {
					cardExp: -7500,
					cost: -30
				};
				...
				const stage = _.mapValues(_.mapValues(_.omitBy(result, (v, k) => k.startsWith('合成-') || k.startsWith('转换-')), v =>
					v < 1 ? 1 : Math.ceil(v)), (v, k) => {
				...
				const stages = _.transform(stage, (r, v, k) => r.push({
					code: k,
					...v
				}), []);
		...
		methods: {
			...
			showPreset(obj, edit = false) {
				...
				if (edit) this.pSetting = _.cloneDeep(this.selected.presets[obj.index].setting);
					...
					_.each(this.elite[this.selectedPresetName].skills.elite, ({
						need
					}, i) => {
			...
			async initPlanner() {
				...
				for (const m of this.penguinData.data.matrix) {
					...
					if (!this.dropTable[code]) this.dropTable[code] = {
						cost: apCost,
						event: stageType === 'ACTIVITY',
						cardExp: 0
					};
				...
				_.forEach(this.dropTable, (drop, code) => {
					this.dropInfo.stageValue[code] = _.sum(_.map(_.omit(drop, dropTableOtherFields), (p, n) => eap[n].value * p)) /
						drop.cost;
				});
